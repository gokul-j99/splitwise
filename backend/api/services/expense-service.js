...
export const personsl_expense_update = async (req,res) =>{
    ...
    if(exp == null){
        return res.status(404).body({"message" : "No expense found"})
    }
    ...
    const group__exp = await expense.findOneAndUpdate(filter,exps,{
        new: true
    }).exec();
    return group__exp;
...
export const group_expense_delete = async (req,res) =>{
    ...
    const expen = await group_expense.findOneAndDelete(filter).exec();
